...
public class DelayConsumer {
    ...
    public void receiveDelay(Channel channel, Message message){
        try {
            ...
            if(taskMsg.getIsFanout()){
                FanoutMsg fanoutMsg = new FanoutMsg(taskMsg.getOperation().name(), taskMsg.getFanoutOpt());
    ...
    @RabbitListener(queues = Constants.CONTENT_FANOUT_DELAY_QUEUE)
    public void receiveFanoutDelay(Channel channel, Message message){
        try {

            String msg=new String(message.getBody());
            JSONObject msgObj=JSONObject.parseObject(msg);

            log.info("receiveFanoutDelay rabbit mq receive a delay message{}", msg.toString());
            TaskMsg taskMsg = JSONObject.toJavaObject(msgObj, TaskMsg.class);
            FanoutMsg fanoutMsg = new FanoutMsg(taskMsg.getOperation().name(), taskMsg.getFanoutOpt());
            producer.sendFanoutMsg(fanoutMsg);
            ...
            try {
                ...
                log.error("receiveFanoutDelay Ack Fail ");
            }
        }
    }
}
